...
TM_RUST_PROVIDER_COUNT := $(words $(filter 1,$(QSOE_RUST_TM_CPIO) $(QSOE_RUST_TM_CRED) $(QSOE_RUST_TM_ELF) $(QSOE_RUST_TM_PROCFS) $(QSOE_RUST_TM_PSEUDODEV) $(QSOE_RUST_TM_RSRCDB) $(QSOE_RUST_TM_SCRIPT) $(QSOE_RUST_TM_SYSCFG) $(QSOE_RUST_TM_SYSFS)))
ifneq ($(filter 2 3 4 5 6 7 8 9,$(TM_RUST_PROVIDER_COUNT)),)
...
.PHONY: all prepare component-overrides clean nvme nvme-populate virtio fsqrv-image tree \
        ...
        check-tm-cpio-model check-tm-cred-model check-tm-elf-model check-tm-procfs-model check-tm-rsrcdb-model \
        ...
        rust-tm-cpio-provider rust-tm-cred-provider rust-tm-elf-provider rust-tm-procfs-provider \
        rust-tm-rsrcdb-provider rust-tm-script-provider rust-tm-syscfg-provider rust-tm-sysfs-provider \
        rust-tm-pseudodev-provider \
        tm-cpio-evidence tm-cred-evidence tm-elf-evidence tm-procfs-evidence tm-rsrcdb-evidence tm-script-evidence \
        ...
        container-rust-tm-elf-provider container-rust-tm-procfs-provider container-rust-tm-rsrcdb-provider container-rust-tm-script-provider \
        container-rust-tm-syscfg-provider container-rust-tm-sysfs-provider \
        container-rust-tm-pseudodev-provider \
        container-tm-cpio-evidence container-tm-cred-evidence container-tm-elf-evidence container-tm-procfs-evidence \
