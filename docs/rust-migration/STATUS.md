...
Only `slogger`, `devb-virtio`, `pipe`, `test_msgpass`, and `tm_procfs` have
Rust-default release-candidate paths. No tracked component has reached
`Retired` status. C remains the rollback path for every current migration
candidate.
