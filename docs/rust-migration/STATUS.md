...
| `pipe` | Yes: `/sbin/pipe` remains C in normal images. | Yes: `QSOE_RUST_PIPE=1 make pipe-artifact`; `make rust-pipe-link-smoke`; `make rust-pipe-smoke`; `make rust-pipe-data-smoke`. | No | No | C mini-spec, C smoke, `qsoe-pipe` host tests, selector, link smoke, ELF audit, Rust-selected registration boot smoke, Rust-selected pipe(2) data-path smoke, and trusted `main` CI run `28102250069` for #96. | Separate Rust-default RC proposal with C rollback if desired. |
