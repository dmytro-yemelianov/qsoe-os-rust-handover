...
| --- | --- | --- | --- | --- | --- | --- |
| Host qrvfs tools | Yes: `host_tools/treeqrvfs.c` remains the fixture oracle and rollback inspector; `host_tools/mkfs-qrv.c` remains the production image writer. | Yes: Rust `qrvfs-tree` runs through `make check-qrvfs-rust-fixture`; Rust `mkfs-qrv-rs` runs through `make check-qrvfs-rust-writer-fixture`, including double-indirect allocation coverage, bounded triple-indirect allocator coverage, target initialization tests, `make check-qrvfs-rust-writer-production-root`, and `make rust-mkfs-qrv-live-smoke`. | RC: `make tree` selects Rust `qrvfs-tree` by default through `build/treeqrvfs`; `make treeqrvfs-rc-rollback-smoke` restores C. Rust writer is opt-in via `QSOE_RUST_MKFS_QRV=1`, not default. | No | qrvfs behavior spec, Rust parser tests, C/Rust fixture comparison, selected artifact smoke, Rust writer fixture, production-root writer comparison, stale-target initialization test, bounded triple-indirect allocator test, Rust-written live virtio `/usr` smoke, and `TREEQRVFS_RC.md`. | Keep C writer default until Rust has a default-writer RC with rollback; keep C rollback until #26's retirement checklist and a separate removal PR are satisfied. |
...
still opt-in writer work until a default-writer RC with rollback exists. No
