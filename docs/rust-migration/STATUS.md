...
Captured: 2026-06-28 22:15 CEST.
...
| --- | --- | --- | --- | --- | --- | --- |
| Host qrvfs tools | Yes: `host_tools/treeqrvfs.c` remains the fixture oracle and rollback inspector; `host_tools/mkfs-qrv.c` remains the production image writer. | Yes: Rust `qrvfs-tree` runs through `make check-qrvfs-rust-fixture`; Rust `mkfs-qrv-rs` runs through `make check-qrvfs-rust-writer-fixture`, including double-indirect allocation coverage, target initialization tests, and `make check-qrvfs-rust-writer-production-root`. | RC: `make tree` selects Rust `qrvfs-tree` by default through `build/treeqrvfs`; `make treeqrvfs-rc-rollback-smoke` restores C. Rust writer is not default. | No | qrvfs behavior spec, Rust parser tests, C/Rust fixture comparison, selected artifact smoke, Rust writer fixture, production-root writer comparison, stale-target initialization test, and `TREEQRVFS_RC.md`. | Keep C writer until Rust has direct block-device/live image evidence and triple-indirect stress coverage; keep C rollback until #26's retirement checklist and a separate removal PR are satisfied. |
