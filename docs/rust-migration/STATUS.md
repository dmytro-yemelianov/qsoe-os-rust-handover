...
Captured: 2026-06-24 13:29 CEST.
...
| `tm_procfs` | Yes: `libtaskman/src/tm_procfs.c` remains selected. | No: boundary reserves future `QSOE_RUST_TM_PROCFS=1`. | No | No | Task-manager inventory, C/Rust boundary review, `make check-tm-procfs-model` host model tests, and C `make procfs-smoke` coverage. | Add a Rust provider behind `QSOE_RUST_TM_PROCFS=1`, then artifact audit, boot smoke, and procfs smoke. |
