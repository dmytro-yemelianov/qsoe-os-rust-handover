...
| `libtaskman` | 16 | 1,932 | Best source of host-testable task-manager modules. `tm_procfs`, `tm_cpio`, `tm_cred`, `tm_script`, `tm_elf`, `tm_syscfg`, and `tm_sysfs` are retired to Rust through the shared provider archive; `tm_pathmgr` is a Rust-default RC with C rollback; remaining candidates are tracked in #153. |
...
`tm_syscfg`, `tm_sysmap`, and `tm_sysfs`. `tm_fdt`, `tm_pathmgr`,
