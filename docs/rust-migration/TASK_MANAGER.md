...
`tm_syscfg`, `tm_sysmap`, and `tm_sysfs` are retired to Rust; `tm_cred` and
`tm_fdt` are Rust-default RCs with C rollback; the rest remain Rust opt-in only. Keep broader loader and
