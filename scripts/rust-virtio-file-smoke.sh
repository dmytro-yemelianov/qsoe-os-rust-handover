#!/usr/bin/env bash
#
# Boot QSOE/L with the opt-in Rust virtio block driver and a temporary on-disk
# sysinit fragment that reads a file from /usr.
...
if [ -z "$log" ]; then
    log="$workdir/boot-smoke-lq-rust-virtio-file.log"
elif [ "${log#/}" = "$log" ]; then
    log="$ROOT/$log"
fi
...
echo "rust-virtio-file-smoke.sh: booting with temporary /usr file-read fragment"
